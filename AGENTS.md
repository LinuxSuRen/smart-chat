...
smart-chat 的使用者是**非技术人员**（如运维值班员、业务操作员），不是开发者。任何改动在
设计 UI 文案、错误提示、交互流程时必须遵守：

- 界面用中文，语气友好；不出现技术词汇：`mcp__`、`stdio`、`streamable-http`、
  `session`、`fiber`、`token`（按钮/标题可用"登录"替代）、HTTP 状态码、堆栈、原始报错。
- 原始错误/诊断信息只进 `servers.json` 的 `logs`/`error` 字段（供排查），
  **永不直接渲染到对话流或弹框**；用户可见的一律替换为"发送失败，请重试"这类可行动的短句。
- 工具名展示时剥掉 `mcp__<server>__` 前缀，server 名以徽标形式呈现（参考
  `web/src/components/Feed.tsx` 的 `displayToolName`）。
- 登录等凭据交互遵循"隐形认证"：有记住的凭据或可自动续期时全程无感；只在用户发消息
  且确实缺认证时弹框，登录成功后自动续发被挂起的消息，不展示技术原因。
