...
<div id="modal"><div class="card">
  ...
  <p id="modal-desc" style="display:none;color:var(--muted);font-size:12.5px"></p>
...
let modalMode = 'token';
function askToken(kind, serverName) {
  modalKind = kind;
  modalMode = 'token';
...
function askBridgeToken() { return askToken('bridge', undefined).then((r) => { if (r.cred.kind === 'token') token = r.cred.token; }); }
...
async function submitServerCredential(serverName, cred, auto) {
  ...
  if (!auto) await promptServerCredential(serverName);
  return false;
}
async function promptServerCredential(serverName) {
  tokenTried.delete(serverName);
  tokenWaiterServer = serverName;
  const r = await askToken('server', serverName);
...
function openEvents() {
  ...
  es.addEventListener('tool_images', (ev) => {
    const d = JSON.parse(ev.data);
    const t = tools.get(d.callId);
    if (!t || !Array.isArray(d.images)) return;
    if (t.imagesSeq === d.seq) return;
    t.imagesSeq = d.seq;
    for (const src of d.images) {
      const img = document.createElement('img');
      img.src = src; img.alt = 'tool result'; img.loading = 'lazy';
      img.style.maxWidth = '100%'; img.style.maxHeight = '320px';
      img.style.borderRadius = '8px'; img.style.border = '1px solid var(--line)';
      img.style.display = 'block';
      t.det.appendChild(img);
    }
    t.det.open = true;
  });
  ...
  es.addEventListener('credential_required', (ev) => {
    ...
    else void promptServerCredential(d.serverName);
  });
...
async function send() {
  ...
  try {
    ...
    if (r.status === 409 && r.data?.code === 'credentials-required') {
      ...
      void promptServerCredential(servers[0] ?? 'MCP server');
      return;
    }
...
function renderServers() {
  ...
  for (const s of lastServers) {
    ...
    if (needsToken) row.querySelector('.tok').onclick = () => { void promptServerCredential(s.serverName); };
