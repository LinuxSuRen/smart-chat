...
<div id="modal"><div class="card">
  ...
  <p style="color:var(--muted);font-size:12.5px">This bridge is protected. Paste the token from the bridge config.</p>
...
$('token-save').onclick = () => {
  ...
  if ($('token-remember').checked) { try { localStorage.setItem('smart-chat.token', token); } catch {} }
  $('modal').classList.remove('open');
  if (tokenWaiter) { const w = tokenWaiter; tokenWaiter = null; w(); }
};
...
async function api(path, opts = {}) {
  for (let attempt = 0; ; attempt++) {
    ...
    const res = await fetch(PREFIX + path, { method: opts.method ?? 'GET', headers, body: opts.body });
    if (res.status === 401 && attempt === 0) { await askToken(); continue; }
    return res;
  }
}
...
async function pollServers() {
  ...
  if (r.status !== 200) { $('serverbar').textContent = 'servers: unavailable (' + r.status + ')'; return; }
...
function renderServers() {
  ...
  for (const s of lastServers) {
    ...
    row.className = 'row';
    row.innerHTML = '<span class="tname"></span><span class="state"></span><span></span><button>remove</button>';
    ...
    row.querySelector('button').onclick = () => { void replaceServers(lastServers.filter((x) => x.serverName !== s.serverName).map((x) => x.entry)); };
