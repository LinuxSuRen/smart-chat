...
// per-SERVER MCP token (401/403 from the target MCP server). Server tokens
// are never written into the server config — the bridge keeps them in host
// memory and merges them into the transport at mount time; this page only
// caches them in localStorage for convenience.
let token = '';
let mcpTokens = {};
...
let tokenWaiter = null;   // resolves with { kind, serverName?, value }
let modalKind = 'bridge';
function askToken(kind, serverName, reason) {
  modalKind = kind;
  $('modal-title').textContent = kind === 'server'
    ? 'Token required: ' + serverName
    : 'Access token required';
  $('modal-desc').textContent = kind === 'server'
    ? (reason || 'This MCP server rejected the request as unauthorized (401/403). Paste its token; it is stored in the bridge memory, not in the server config.')
...
$('token-save').onclick = () => {
  ...
  if ($('token-remember').checked) {
    if (modalKind === 'server' && tokenWaiterServer) mcpTokens[tokenWaiterServer] = value, saveMcpTokens();
    else if (modalKind === 'bridge') { try { localStorage.setItem('smart-chat.token', value); } catch {} }
  }
  $('modal').classList.remove('open');
  if (tokenWaiter) { const w = tokenWaiter; tokenWaiter = null; w({ kind: modalKind, value }); }
};
...
function askBridgeToken() { return askToken('bridge', undefined, undefined).then((r) => { token = r.value; }); }

// ---- per-server token flow ---------------------------------------------
const tokenTried = new Set();
async function submitServerToken(serverName, tok, auto) {
  tokenTried.add(serverName);
  try {
    const r = await apiJson('/servers/' + encodeURIComponent(serverName) + '/token', { method: 'POST', body: JSON.stringify({ token: tok }) });
    if (r.status === 200) {
      sysLine('token accepted for ' + serverName + ' — please retry the request');
      void pollServers();
      return true;
    }
    sysLine('token rejected for ' + serverName + ': ' + (r.data?.error ?? r.status));
  } catch (err) {
    sysLine('token submit failed for ' + serverName + ': ' + (err && err.message ? err.message : String(err)));
  }
  if (!auto) await promptServerToken(serverName, 'retry');
  return false;
}
async function promptServerToken(serverName, reason) {
  ...
  if (r.value !== '') await submitServerToken(serverName, r.value, false);
}
async function maybeAutoSubmit(serverName) {
  if (tokenTried.has(serverName)) return;
  const stored = mcpTokens[serverName];
  if (stored) await submitServerToken(serverName, stored, true);
}
...
function openEvents() {
  ...
  es.addEventListener('turn_done', () => { endAssistant(); connDot.classList.add('ok'); });
  es.addEventListener('credential_required', (ev) => {
    const d = JSON.parse(ev.data);
    const stored = mcpTokens[d.serverName];
    if (stored && !tokenTried.has(d.serverName)) void submitServerToken(d.serverName, stored, true);
    else void promptServerToken(d.serverName, d.reason);
  });
...
function renderServers() {
  ...
  for (const s of lastServers) {
    ...
    st.textContent = needsToken ? 'needs token' : s.state;
    st.className = 'state ' + (needsToken ? 'failed' : s.state);
    row.children[2].textContent = (s.toolCount ?? 0) + ' tools' + (s.error ? ' — ' + s.error : '');
    if (needsToken) row.querySelector('.tok').onclick = () => { void promptServerToken(s.serverName, s.auth.reason); };
