    ...
    const handle = async (req, res) => {
      ...
      try {
        ...
        if (sub === '/messages') {
          ...
          // this exact text once credentials land). Servers configured with
          // auth.probeTool get a SILENT probe first — a read-only tool call
          // that touches the platform — so the dialog appears at input time
          // instead of after a failed model tool call, and servers with
          // static credentials never prompt at all.
          const blocked = typeof manager.probeGate === 'function'
            ? await manager.probeGate(entry.agent)
            : (typeof manager.authBlocked === 'function' ? manager.authBlocked() : [])
          if (blocked.length > 0) {
