  ...
  ctx.inject(['webServer'], (sctx) => {
    ...
    const handle = async (req, res) => {
      ...
      try {
        ...
        const tokenMatch = /^\/servers\/([^/]+)\/token$/.exec(sub)
        if (tokenMatch !== null) {
          if (method !== 'POST') return sendJson(res, 405, { error: 'method not allowed' })
          let body
          try {
          ...
          if (manager.setToken === undefined) return sendJson(res, 501, { error: 'token flow unavailable' })
          const verdict = manager.setToken(decodeURIComponent(tokenMatch[1]), typeof body.token === 'string' ? body.token : '')
