...
export function createServerManager(ctx, config, log) {
  ...
  // --- per-server bearer tokens (never persisted to settings) -----------
  // Streamable-http MCP servers that answer 401/403 without credentials get
  // their token from the CHAT PAGE (input dialog), kept in host memory only
  // and merged into the transport headers at mount time. The settings entry
  // itself stays credential-free.
  const AUTH_NEEDED_PATTERN = /\b(401|403)\b|unauthorized|forbidden/i
  const authTokens = new Map()    // serverName -> bearer token
  const authRequired = new Map()  // serverName -> reason string (watchdog flag)

  /** Merge a stored token into a streamable-http entry WITHOUT mutating it. */
  const mergedEntry = (entry) => {
    if (entry?.transport !== 'streamable-http') return entry
    const token = authTokens.get(entry.serverName)
    if (token === undefined || token === '') return entry
    return { ...entry, headers: { ...(entry.headers ?? {}), Authorization: `Bearer ${token}` } }
  }

  /** Store (or clear, "") a server token and remount the fiber immediately. */
  const setToken = (serverName, token) => {
    ...
    if (entry.transport !== 'streamable-http') {
      return { ok: false, status: 400, error: 'only streamable-http servers use tokens' }
    }
    if (typeof token !== 'string' || token.length > 4096) {
      return { ok: false, status: 400, error: 'invalid token' }
    }
    if (token === '') authTokens.delete(serverName)
    else authTokens.set(serverName, token)
    authRequired.delete(serverName)
    serverLogs.delete(serverName)
    void remount(serverName, token === '' ? 'token cleared' : 'token updated')
    return { ok: true }
  }
  ...
  const authStateFor = (serverName, record) => {
    ...
    // even while the connection itself is healthy. setToken clears it.
    const flagged = authRequired.get(serverName)
    if (flagged !== undefined) return { required: true, reason: flagged }
    if (record !== undefined && (toolCounts.get(serverName) ?? 0) > 0) return { required: false }
    const texts = []
    ...
    if (AUTH_NEEDED_PATTERN.test(texts.join(' '))) {
      return { required: true, reason: 'connection or tool call rejected as unauthorized' }
    }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (AUTH_NEEDED_PATTERN.test(failure)) {
      // Unauthorized tool call: do NOT remount (the connection is fine) —
      // surface the need so the page can ask for a token, then setToken()
      // performs the remount with merged credentials.
      const reason = `tool call rejected as unauthorized (${text.slice(0, 120) || '401/403'})`
      if (authRequired.get(prefix[1]) !== reason) {
        authRequired.set(prefix[1], reason)
        noteServerLog(prefix[1], reason)
        log.warn('server "%s": %s; supply a token from the chat page', prefix[1], reason)
        try { api.onCredentialNeeded?.(prefix[1], reason) } catch (error) { log.warn(error) }
      }
    }
  })
