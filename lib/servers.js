...
export function validateServerEntry(entry) {
  ...
  if (entry.transport === 'stdio') {
    ...
    if (entry.auth !== undefined) {
      ...
      // Optional login endpoint kept for backward compatibility with older
      // deployments; the passthrough flow never uses it.
...
export function createServerManager(ctx, config, log) {
  ...
  //   token    — pasted bearer token: X-Platform-Token + Authorization
  //   password — username + password, PURE PASSTHROUGH: the MCP server
  //              receives them (X-Platform-Username/X-Platform-Password)
  //              and runs its own authentication/JWT lifecycle; smart-chat
  //              confirms success by executing a tool (auth.probeTool or
  //              the model's calls), never by logging in itself.
  ...
  const credentials = new Map() // serverName -> { kind:'token', token } | { kind:'password', username, password, passthrough: true }
  ...
  // NOTE: smart-chat never performs logins itself. Username/password
  // credentials are PASSTHROUGH: they ride the MCP requests as
  // X-Platform-Username/X-Platform-Password and the MCP server runs its own
  // authentication/JWT lifecycle. Whether they work is confirmed the only
  // reliable way — by executing a tool (auth.probeTool / the model's calls).

  /** Merge stored credentials into a streamable-http entry WITHOUT mutating it.
   *
   * Credentials ride the MCP requests in the robot-platform per-call
   * vocabulary (unknown X-Platform-* headers are ignored by servers that
   * do not read them): X-Platform-Token (token) or X-Platform-Username/
   * X-Platform-Password (passthrough account); the server pins them to
   * the MCP session at initialize. Token-kind ALSO sets Authorization
   * for generic deployments without admission.
   */
  const mergedEntry = (entry) => {
    ...
    } else if (cred.kind === 'password') {
      // Pure passthrough: the MCP server runs its own auth/JWT lifecycle
      // from these headers. Authorization is left untouched so admission
      // tokens configured via entry headers keep working.
   ...
   * username/password pair (both pure passthrough — see the header comment).
   */
  const setCredentials = async (serverName, body) => {
    ...
    if (hasToken) {
      ...
      // Password mode is pure PASSTHROUGH: no bridge-side login, no login
      // URL. The credentials ride the MCP requests (X-Platform-Username/
      // X-Platform-Password) and the MCP server runs its own auth/JWT
      // lifecycle; correctness is confirmed by the silent probe / the
      // model's tool calls, never by guessing a login endpoint.
      credentials.set(serverName, { kind: 'password', username: body.username, password: body.password, passthrough: true })
    }
    // New credentials: the silent probe must re-verify them (the auto
    // re-sent message re-enters the gate).
    probeVerified.delete(serverName)
    authRequired.delete(serverName)
    // Await the remount: the auto re-sent message re-probes immediately, and
    // probing while the OLD (credential-less) fiber still owns the registered
    // tools would hit the old session, fail, and loop the login dialog.
    await remount(serverName, hasToken ? 'token updated' : 'credentials updated')
    // Clear the diagnostics only AFTER the old fiber is fully disposed: its
    // final failing retries would otherwise repopulate the ring (401s from
    // the credential-less generation) and re-flag the server as needing a
    // login right after a successful credential save.
    serverLogs.delete(serverName)
    return { ok: true }
  }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (PER_CALL_CRED_PATTERN.test(failure)) {
      ...
      // credentials — our credential domain. Passthrough credentials mean
      // the server already retried internally; a persistent rejection means
      // the stored account/token is wrong → re-verify and prompt. Raw error
      // text goes to the server diagnostics only, never to the UI.
      probeVerified.delete(prefix[1])
      noteServerLog(prefix[1], `server asked the caller for per-request credentials: ${text.slice(0, 160)}`)
      flagAuthNeeded(prefix[1], 'login required')
      return
    }
    if (AUTH_NEEDED_PATTERN.test(failure)) {
      ...
      // own platform credentials or per-API permissions of the target system
      // (passthrough accounts are refreshed BY the server internally). That
      // is the server's credential domain, not ours: surface it as a normal
      // tool error and NEVER prompt. (The note must stay free of the trigger
      // words — authStateFor scans these logs for connection-level
      // rejections.)
  ...
  const probeCredentials = async (entry, agent) => {
    ...
    if (failureText !== '' && (PER_CALL_CRED_PATTERN.test(failureText) || AUTH_NEEDED_PATTERN.test(failureText))) {
      noteServerLog(serverName, `silent probe rejected: ${failureText.slice(0, 160)}`)
      flagAuthNeeded(serverName, 'login required')
      return { blocked: true }
    }
    if (failureText !== '') {
