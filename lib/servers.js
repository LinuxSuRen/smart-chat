...
export function createServerManager(ctx, config, log) {
  ...
  const mergedEntry = (entry) => {
    ...
    if (cred.kind === 'token' && cred.token !== '') {
      ...
      // Dual form: Bearer (old middleware) + Cookie (new middleware).
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (AUTH_NEEDED_PATTERN.test(failure)) {
      // Unauthorized: if password credentials are stored, re-login quietly
      // and remount (mirrors robot-platform-mcp's own JWT fallback). Only
      // when that is impossible/failing do we surface a prompt so the page
      // can ask for a token or username/password.
      ...
      flagAuthNeeded(prefix[1], `tool call rejected as unauthorized (${text.slice(0, 120) || '401/403'})`)
    }
  })
